...
namespace Gram.Application.Attendees.Commands
{
    public class UpdateAttendanceCommand : IRequest
    {
        ...
        public class Handler : BaseHandler, IRequestHandler<UpdateAttendanceCommand, Unit>
        {
            ...
            {
                if ((await DataContext.Attendees.AsNoTracking().FirstOrDefaultAsync(m => m.Id == request.Model.Id, cancellationToken)) == null)
                    throw new EntityNotFoundException(nameof(Attendance), request.Model.Id);
